...
- **Feito:** Planos 01 (foundation), 02 (render primitives), 03a (format/builder primitives). ~69 testes, clippy clean. ~21 commits desde a base.
...
- **Pendência aberta:** JSON `remaining` em Rust serializa `60.0` (f64) vs TS `60` (inteiro). Dentro do contrato (JSON é parseado, não byte-exact). **Decidir no Plano 3c** (passo de golden snapshot de JSON): aceitar `60.0` OU modelar `remaining`/`used` como inteiro p/ bater byte-a-byte.
...
- **03b — builders por-provider:** `builders/{claude,codex,amp,generic}.rs` + `normalize_plan_label`. Lê: `src/formatters/builders/{claude,codex,amp,generic}.ts`, `src/formatters/codex-helpers.ts`. (Mapa já foi feito 1× num Workflow — re-ler o TS é a fonte da verdade.) Consome os primitivos do 3a (`model_line`, `header_line`, `label_line`, `vline`, `build_footer_line`). Atenção: Codex windowPolicy/severity-sort; Claude weeklyModels(Opus/Sonnet/Cowork)+extraUsage(centavos, gate limit>0); Amp free-tier layouts (sublines/inline/generic).
- **03c — assembly + golden:** `view_model.rs`, `waybar.rs` ({text,tooltip,class} + cache 5s settings + hidden-module short-circuit), `terminal.rs`. **PASSO 0: gerar golden snapshots do TS** (`tests/__snapshots__/formatters-snapshot.test.ts.snap` é a referência byte-exact; portar como `insta`). **Aqui valida a paridade total e decide o 60.0/60.** Class-token format: single=`agent-bar-<provider> <status>`; aggregate=`agent-bar` + `<provider>-<status>`.
