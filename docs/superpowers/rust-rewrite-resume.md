...
- **Feito:** Planos 01-03c (formatação) + **04a (infra providers + Claude)** + **04b (Amp)**. **225 testes** (5 suítes, 58 golden), clippy clean. Branch @ `3706d19`. Reviews de branch Opus 04a/04b = "sólido, SIM" (zero Critical). **Camada de formatação + infra async + 2 providers reais (Claude HTTP, Amp subprocess) prontos.** Próximo: **Plano 04c (Codex — app-server JSON-RPC + fallback; a fase mais difícil)**, depois 04d (notify).
