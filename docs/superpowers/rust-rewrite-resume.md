...
- **Feito:** Planos 01-03c (formatação) + **04a (infra + Claude)** + **04b (Amp)** + **04c (Codex)**. **278 testes** (5 suítes, 58 golden), clippy clean. Branch @ `c21a44c`. Reviews de branch Opus 04a/04b/04c = "sólido, SIM" (zero Critical). **Camada de formatação + infra async + os 3 providers reais (Claude HTTP, Amp subprocess, Codex app-server+fallback) completos no `registry()`.** Próximo: **Plano 04d (notify) — ÚLTIMO do Plano 4**, depois 5 (CLI/main), 6 (install), 7 (dist/cutover).
...
- **04d — notify — PRÓXIMO (ÚLTIMO do Plano 4; autorar lendo `src/notify.ts`).** Fundação CONFIRMADA suficiente (zero mudança de Ctx). `notify.rs`: `plan_notifications(&AllQuotas, &HashMap<String,ProviderNotifyState>) -> NotifyPlan` PURO (escalação por rank `{ok:0,low:1,critical:2}`, dedup de alias por `(round(used), resetsAt)`, re-arm em recuperação sem disparar, sanitiza estado stale→ok) + `check_and_notify` ASYNC IO (lê/grava estado por-provider `notify-<provider>.json` atômico em `ctx.paths.cache_dir`, spawn `notify-send` via `tokio::process` best-effort, **persistir estado SÓ p/ os `changed` após envio**, no-op se `notify-send` ausente). Thresholds `LOW_USED=90`/`CRITICAL_USED=95` (sobre %usado). `windowsOf`: models + weeklyModels (via `get_claude_extra`) + primary/secondary, dedup por sig. Gates (stdout não-TTY, `notify.enabled`, comando=waybar, !json, !watch) ficam no CALLER (Plano 5). Testar `plan_notifications` puro (síncrono) + `check_and_notify` (tempdir + fake notify-send).
