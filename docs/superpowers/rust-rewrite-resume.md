...
- **03c — assembly + golden: AUTORADO (commit 8881043), PRONTO P/ EXECUTAR.** Plano: `docs/superpowers/plans/2026-06-19-rust-rewrite-03c-assembly-golden.md`. 4 tasks: T1 prep (HealthStatus::as_str + APP_BASE_CLASS), T2 `terminal.rs`, T3 `waybar.rs` (WaybarOutput/format_for_waybar/format_provider_for_waybar), T4 golden snapshots (insta + filters de sanitização + Clock fixo, paridade byte-exact vs `.snap` do TS). Funções de assembly PURAS (recebem &Settings + &Clock injetados). **Cache 5s settings + hidden-module short-circuit + outputX(console.log) DEFERIDOS p/ Plano 5.** Decisão 60.0/60: aceitar 60.0 (sem golden JSON — não-issue). Executar via loop §3.
