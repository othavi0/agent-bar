...
- **Feito:** Planos 01 (foundation), 02 (render primitives), 03a (format/builder primitives), **03b (builders por-provider — extras getters, codex model-prep, CodexViewModel, normalize_plan_label, build_{generic,claude,codex,amp})**. **94 testes**, clippy clean. Branch @ `08684ad`.
...
- **03c — assembly + golden (PRÓXIMO):** `view_model.rs` (assembly geral), `waybar.rs` ({text,tooltip,class} + cache 5s settings + hidden-module short-circuit), `terminal.rs`. Lê: `src/formatters/{waybar,terminal,view-model}.ts` + `src/formatters/__snapshots__`. Os builders do 3b são chamados como `build_X(clock, p, [view_model,] &BuildOptions)` — BuildOptions já cobre tudo (sem retrabalho de assinatura). **PASSO 0: gerar golden snapshots do TS** (`tests/__snapshots__/formatters-snapshot.test.ts.snap` é a referência byte-exact; portar como `insta`). **Aqui valida a paridade total.** Class-token format: single=`agent-bar-<provider> <status>`; aggregate=`agent-bar` + `<provider>-<status>`. **Recomendações do review de branch 03b:** (a) construir os fixtures dos golden com inputs em ordem BTreeMap (ou documentar a variante TUI onde `Object.entries` pode divergir de BTreeMap); (b) incluir um caso de sort Codex com nomes ASCII; (c) documentar a invariante "`error` nunca string vazia" do lado do provider (M3); (d) aplicar a decisão 60.0/60 (aceitar 60.0, comparar JSON por valor parseado).
