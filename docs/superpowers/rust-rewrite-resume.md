...
- **Feito:** Planos 01 (foundation), 02 (render primitives), 03a (format/builder primitives), 03b (builders por-provider), 03c (assembly terminal/waybar + golden byte-exact), **04a (provider infra async/tokio + ClaudeProvider)**. **208 testes** (5 suítes, 58 golden), clippy clean. Branch @ `24415c4`. 04a review de branch Opus = "sólido p/ Amp/Codex/notify, SIM" (zero Critical; Minors deferidos). **Toda a camada de formatação + a infra de providers async + o 1º provider real (Claude) prontos.** Próximo: **Plano 04b (Amp)**, depois 04c (Codex — o mais difícil), 04d (notify).
...
- **04b — Amp — PRÓXIMO (autorar lendo `src/providers/amp.ts` + `src/amp-cli.ts`).** Estende `QuotaSource` (template do 04a). Recomendações do review de branch 04a: `QuotaSource::Raw=String` (stdout do `amp usage`); `fetch_raw` via `tokio::process::Command` + `.kill_on_drop(true)` + `tokio::time::timeout(5s)` + **drenar stderr concorrente** (senão deadlock de pipe); env `NO_COLOR=1`/`TERM=dumb` via `.env()` (constantes, não no Ctx); descoberta do binário (`which amp` → `~/.local/bin/amp` → `~/.amp/bin/amp` → `~/.cache/.bun/bin/amp` → `~/.bun/bin/amp`) em `amp_cli.rs` (só o locator; `ensure_amp_cli` interativo é Plano 6); `fullAt` (resetsAt do Free Tier) via `iso_from_ms(ctx.now_ms + millis)`; auth fail (exit≠0 ou sem `Signed in as`) → `Err(AmpError::NotLoggedIn)` ANTES do cache; parse por regex (sem `amp usage --json`). Testar `parse_usage`/`build_quota` com strings mockadas + seam de spawn (script-fake). `error=Some("")` é invariante do provider (não mudar builder). Registrar Amp no `registry()`.
- **04c — Codex (o mais difícil) — (autorar lendo `src/providers/codex.ts`).** Estende `QuotaSource`. `Raw`=struct composto (rate limits do app-server OU do session log). `fetch_raw`: handshake JSON-RPC stdio (`initialize` id 0 com `clientInfo.{name,title,version=ctx.version}` → `initialized`+`account/read` id 1 `params.refreshToken=false`+`account/rateLimits/read` id 2; respostas fora de ordem; grace 200ms se id2 antes do id1; timeout EXTERNO 4s ≠ 5s HTTP; kill no fim) via `tokio::process` + `select!`; fallback: maior-mtime `.jsonl` em `~/.codex/sessions/YYYY/MM/DD` (hoje/ontem via `ctx.local_offset`), scan reverso por `token_count`, `walkdir`/`read_dir` (sem Bun.Glob). Erros → `Err(CodexError::{NoSessionData,NoRateLimitData})` (não cachear); `NoQuotaWindows` é erro embutido no build_quota. `classify_window`/`normalize_plan` já existem em formatters/shared.rs. Codex preenche só `extra` cru (view-model resolvido no assembly). Testar app-server via `tokio::io::duplex`. **models_detailed fica BTreeMap** (Codex re-ordena por severity no codex_helpers).
- **04d — notify — (autorar lendo `src/notify.ts`).** `plan_notifications` (puro: escalação por rank, dedup de alias por `(round(used), resetsAt)`, re-arm em recuperação) + `check_and_notify` (IO: lê/grava estado por-provider `notify-<provider>.json` atômico em `paths.cache_dir`, spawn `notify-send` best-effort, **persistir estado SÓ após envio**). Thresholds `LOW_USED=90`/`CRITICAL_USED=95` (sobre %usado). Usa `get_claude_extra` p/ weeklyModels. Gates (stdout não-TTY, `notify.enabled`, comando=waybar, !json, !watch) ficam no CALLER (Plano 5).
