...
export const COPILOT_MISSING_ERROR = 'GitHub Copilot CLI not installed. Install `copilot` and log in.';
...
export function findCopilotBin(
  options: { home?: string; which?: (cmd: string) => string | null; exists?: (path: string) => boolean } = {},
) {
  const which = options.which ?? (typeof Bun.which === 'function' ? Bun.which : undefined);
